...
const options: swaggerJsdoc.Options = {
  definition: {
      ...
      schemas: {
        ...
        Route: {
          ...
          properties: {
            ...
            cbState: { type: 'string', enum: ['CLOSED', 'OPEN', 'HALF_OPEN'] },
            lbStrategy: { type: 'string', enum: ['ROUND_ROBIN', 'WEIGHTED', 'FAILOVER'] },
            routeGroupId: { type: 'string', nullable: true },
        ...
        RouteBody: {
          ...
          properties: {
            ...
            cbFailureThreshold: { type: 'integer', minimum: 1, maximum: 100, default: 5 },
            cbRecoveryTimeout: { type: 'integer', minimum: 1000, maximum: 300000, default: 30000 },
            lbStrategy: { type: 'string', enum: ['ROUND_ROBIN', 'WEIGHTED', 'FAILOVER'], default: 'ROUND_ROBIN' },
            routeGroupId: { type: 'string', nullable: true },
            organizationId: { type: 'string', nullable: true },
          },
        },
    ...
    tags: [
      ...
      { name: 'Route Groups', description: 'Route group management' },
      { name: 'Organizations', description: 'Multi-tenant organization and team management' },
      { name: 'Targets', description: 'Load balancing targets for routes' },
      { name: 'Transforms', description: 'Request/response transformation rules' },
      { name: 'Incidents', description: 'Incident timeline and management' },
      { name: 'Change Requests', description: 'Change request approval workflow' },
      { name: 'Achievements', description: 'User achievement system' },
      { name: 'Traffic', description: 'Live traffic, traffic map, and PII sanitizer' },
    ],
  },
