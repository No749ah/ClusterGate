  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Manage system configuration and maintenance</p>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          {updateInfo?.updateAvailable && (
            <div className="space-y-3 pt-1">
              ...
              >
                {isUpdating ? (
                  <><Loader2 className="w-4 h-4 animate-spin mr-2" /> {updateSteps.length > 0 ? 'Updating...' : 'Checking...'}</>
                ) : (
              ...
              {/* Update Progress */}
              {(isUpdating || updateSteps.length > 0 || updateComplete) && (
                <div className="rounded-lg border border-border/50 p-4 space-y-3">
                  {/* Progress bar */}
                  <div className="space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">{updateProgress?.label || 'Connecting...'}</span>
                        <span className="text-muted-foreground font-mono">
                          {updateProgress ? `${updateProgress.step}/${updateProgress.totalSteps}` : '0/6'}
                        </span>
                      </div>
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: updateProgress ? `${(updateProgress.step / updateProgress.totalSteps) * 100}%` : '0%' }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="space-y-1">
                    {isUpdating && updateSteps.length === 0 && (
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground">Connecting to update stream...</span>
                      </div>
                    )}
                    {updateSteps.map((s) => (
