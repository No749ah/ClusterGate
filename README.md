...
- **Route Management** — Guided 7-step create/edit wizard with a built-in target reachability check; test, publish, version, duplicate, import/export routes via UI; **archived routes** with restore/permanent-delete (per-org permissions); **bulk actions** (publish/deactivate/delete, set environment, move to group, add tags); **Copy-as-cURL** + JSON export; keyboard shortcuts on the route list (`/` search, `n` new, Ctrl/Cmd+A/C/V, Ctrl/Cmd+K shortcuts, Del); **pinned routes** (inline pin icon on the detail header) and **recent items** for quick jump-back; **redirect-rewriting** keeps upstream 3xx Locations under `/r/…`
- **Readable URLs** — Routes, groups and organizations use auto-generated slugs (`/routes/docs-proxy` instead of `/routes/cm…cuid`); when a slug carries a disambiguating suffix it's hidden behind `?id=` so the path stays readable; old id-based URLs still resolve so bookmarks don't break
- **Navigation polish** — Browser tab title shows the current page/entity, list filters + active tab persist in the URL (bookmarkable, shareable), tag and environment chips filter the list on click, list pages have sticky headers, cached rows-per-page selection, and `X–Y of Z` pagination; dashboard tiles and analytics charts deep-link with the right filter pre-applied; smart 404 page suggests close matches
- **Honest error classification** — Status taxonomy splits into six buckets so 4xx/404 doesn't inflate your error rate: `success` · `client` (4xx) · `throttled` (429) · `maintenance` · `degraded` (circuit breaker) · `error` (upstream 5xx + proxy/SSL/network/gateway-auth failure). Used everywhere — dashboard Error Rate, route Success Rate, analytics charts, /activity filter
- **Activity log** — Free-text search across path/IP/error, date-range, **live tail** (2s auto-refresh), multi-select route + method filters, clickable status/method/route cells that toggle the filter, request/response detail as a modal
